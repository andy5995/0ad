	...
	gloox = {
		compile_settings = function()
			...
			elseif not _OPTIONS["android"] then
				...
				pkgconfig.add_includes("gloox", os.getenv("GLOOX_CONFIG") and " --cflags")
			end
		end,
		link_settings = function()
			if os.istarget("windows") then
			...
			elseif not _OPTIONS["android"] then
				pkgconfig.add_links("gloox", os.getenv("GLOOX_CONFIG") and " --libs")
			end
		end,
	},
	iconv = {
	...
	icu = {
		compile_settings = function()
			...
			elseif not _OPTIONS["android"] then
				...
				-- i.e. on OSX where it gets set in update-workspaces.sh
				pkgconfig.add_includes("icu-i18n", os.getenv("ICU_CONFIG") and " --cppflags")
			end
		end,
		link_settings = function()
			if os.istarget("windows") then
			...
			elseif not _OPTIONS["android"] then
				pkgconfig.add_links("icu-i18n", os.getenv("ICU_CONFIG") and " --ldflags-searchpath --ldflags-libsonly --ldflags-system")
			end
		end,
	},
	libcurl = {
	...
	libxml2 = {
		compile_settings = function()
			...
			elseif not _OPTIONS["android"] then
				...
				-- i.e. on OSX where it gets set in update-workspaces.sh
				pkgconfig.add_includes("libxml-2.0", os.getenv("XML2_CONFIG") and " --cflags")
			end
			if os.istarget("macosx") then
		...
		link_settings = function()
			if os.istarget("windows") then
				...
				filter { }
			elseif not _OPTIONS["android"] then
				pkgconfig.add_links("libxml-2.0", os.getenv("XML2_CONFIG") and " --libs")
			end
		end,
	},
	miniupnpc = {
	...
	sdl = {
		compile_settings = function()
			...
			elseif not _OPTIONS["android"] then
				...
				-- i.e. on OSX where it gets set in update-workspaces.sh
				pkgconfig.add_includes("sdl2", os.getenv("SDL2_CONFIG") and " --cflags")
			end
		end,
		link_settings = function()
			if os.istarget("windows") then
			...
			elseif not _OPTIONS["android"] then
				pkgconfig.add_links("sdl2", os.getenv("SDL2_CONFIG") and " --libs")
			end
		end,
	},
	spidermonkey = {
